...
public interface LUserMessageMapper {
    @Delete({ "delete from l_user_message where h_message_id = #{hMessageId,jdbcType=BIGINT} and h_user_id= #{hUserId,jdbcType=BIGINT}" })
int deleteByPrimaryKey(Map<String, Object> paramMap);

    @Insert({ "insert into l_user_message (l_user_message_id, h_user_id, ", "h_message_id, create_ts, ", "update_ts)",
            "values (#{lUserMessageId,jdbcType=BIGINT}, #{hUserId,jdbcType=BIGINT}, ",
            "#{hMessageId,jdbcType=BIGINT}, #{createTs,jdbcType=TIMESTAMP}, ", "#{updateTs,jdbcType=TIMESTAMP})" })
    ...
    @Select({ "select", "l_user_message_id, h_user_id, h_message_id, create_ts, update_ts", "from l_user_message",
    ...
    @Select({ "select", "l_user_message_id, h_user_id, h_message_id, create_ts, update_ts", "from l_user_message",
    "where h_message_id = #{hMessageId,jdbcType=BIGINT} and h_user_id= #{hUserId,jdbcType=BIGINT}" })
@Results({
    @Result(column = "l_user_message_id", property = "lUserMessageId", jdbcType = JdbcType.BIGINT, id = true),
    @Result(column = "h_user_id", property = "hUserId", jdbcType = JdbcType.BIGINT),
    @Result(column = "h_message_id", property = "hMessageId", jdbcType = JdbcType.BIGINT),
    @Result(column = "create_ts", property = "createTs", jdbcType = JdbcType.TIMESTAMP),
    @Result(column = "update_ts", property = "updateTs", jdbcType = JdbcType.TIMESTAMP) })
List<LUserMessage> selectByMessageIdAndUserId(Map<String, Object> paramMap);

    @Select({ "select max(l_user_message_id) as l_user_message_id from ls_user_message.l_user_message" })
    @Results({ @Result(column = "l_user_message_id", property = "lUserMessageId", jdbcType = JdbcType.BIGINT) })
    ...
    @Update({ "update l_user_message", "set h_user_id = #{hUserId,jdbcType=BIGINT},",
            "h_message_id = #{hMessageId,jdbcType=BIGINT},", "create_ts = #{createTs,jdbcType=TIMESTAMP},",
